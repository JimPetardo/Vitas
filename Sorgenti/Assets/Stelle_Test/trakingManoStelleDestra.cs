...
public class trakingManoStelleDestra : MonoBehaviour {
	...
	void OnDestroy(){
		...
		System.IO.File.WriteAllText("save/StelleHandDestra_"+nome +cognome +ora+".txt", "" );
		for (int p = 0; p < listaPuntiX.Count; p++) // Loop with for.
		{
			System.IO.File.AppendAllText("save/StelleHandDestra_"+nome +cognome +ora+".txt", 

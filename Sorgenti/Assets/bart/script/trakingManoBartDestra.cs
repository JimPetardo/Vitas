...
public class trakingManoBartDestra : MonoBehaviour {
	...
	void OnDestroy(){
		...
		System.IO.File.WriteAllText("save/BartHandDestra_"+nome +cognome +ora+".txt", "" );
		for (int p = 0; p < listaPuntiX.Count; p++) // Loop with for.
		{
			System.IO.File.AppendAllText("save/BartHandDestra_"+nome +cognome +ora+".txt", 
